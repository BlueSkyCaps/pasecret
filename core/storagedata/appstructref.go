...
type AppStructRef struct {
	...
	LocalizedTextFunc   func(messageId string, tmd map[string]interface{}) string
...
func (appRef AppStructRef) RepaintDataListByEdit(cidOrg string) {
	...
	appRef.DataList.UpdateItem = func(i widget.ListItemID, o fyne.CanvasObject) {
		...
		(o.(*fyne.Container).Objects[1]).(*widget.Button).OnTapped = func() {
			dialog.ShowConfirm(AppRef.LocalizedTextFunc("dialogShowInformationTitle", nil),
				AppRef.LocalizedTextFunc("dataListDeShowConfirm", nil),
				func(b bool) {
					if b {
